...
| `gemini-fanout-review.sh` | Same 3 lenses on Gemini via the Antigravity CLI (`agy`), billed to the Google AI Pro/Ultra subscription rather than per token. Select with `ARL_FANOUT_BACKEND=gemini`; tune with `ARL_GEMINI_MODEL` (default `gemini-3.1-pro-high`; `agy models` lists ids). Sends the prompt as stream-json on **stdin**, never argv — `agy -p` takes its prompt as an argument and Windows caps a command line near 32 KB. Inlines touched-file contents since it runs without tool permissions. Fails closed on non-zero exit, a non-`SUCCESS` result status, or a missing `VERDICT:` line. Note: `gemini` (Gemini CLI) itself is **not** usable here — Google cut individual plans off on 2026-06-18 and it now answers OAuth with `IneligibleTierError`/`UNSUPPORTED_CLIENT`. |
| `gate.sh` | **The gate's entry point.** Picks the panel and the runner, then hands off. `ARL_GATE=panel` (default) is the mixed panel; `nocodex` drops to an all-OpenRouter panel that keeps two model families, for when the ChatGPT subscription is out of usage; `single` puts all three lenses on one model, picked to be a *different* family from the author (Claude author → codex reviewer, codex author → Claude reviewer); `ARL_FORCE_CODEX_FANOUT=1` pins codex. The only per-host difference is the default repo_dir, supplied as `ARL_DEFAULT_REPO` by a thin `~/fanout-review.sh` shim, so both machines run identical gate logic. |
| `lenses.sh` | Shared correctness/data/ui lens prompts + VERDICT rules, **and the shared safety preamble** (`arl_abs`, `arl_pick_python`), sourced by every backend so neither the wording nor the argument handling can drift between them. `arl_abs` makes the diff and out-prefix absolute before any backend cds to the repo — a relative diff read after that cd yields an EMPTY review that a model can APPROVE. `arl_pick_python` picks an interpreter that actually *runs*, since Windows' `python3` is an App Execution Alias that resolves and then fails to launch. |
